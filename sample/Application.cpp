...
void Application::render_chat_box() {
	...
	for (auto chat : chat_history) {
		...
		if (!userAsk.empty()) {
			...
			ImVec2 text_size = ImGui::CalcTextSize(userAsk.c_str());
			...
			ImGui::InputTextMultiline(("##" + userAsk + std::to_string(num)).c_str(),
				const_cast<char*>(userAsk.c_str()), userAsk.size() + 1, input_size,
